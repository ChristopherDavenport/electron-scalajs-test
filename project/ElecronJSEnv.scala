...
final class ElectronJSEnv(config: ElectronJSEnv.Config) extends JSEnv {
  import ElectronJSEnv._

  def this() = this(ElectronJSEnv.Config())

  val name: String = "Electron"

  def start(input: Seq[Input], runConfig: RunConfig): JSRun = {
    ElectronJSEnv.validator.validate(runConfig)
    validateInput(input)
  ...
  private def internalStart(input: Seq[Input], runConfig: RunConfig): JSRun = {
    ...
    ExternalJSRun.start(command, externalConfig)(ElectronJSEnv.write(input))
  }
...
object ElectronJSEnv {
  ...
  object Config {
    /** Returns a default configuration for a [[ElectronJSEnv]].
     *
     *  The defaults are:
     *
     *  - `executable`: `""`
